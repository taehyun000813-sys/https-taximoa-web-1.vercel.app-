...
<head>
  ...
  <style>
    ...
    * { box-sizing: border-box; margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, "Apple SD Gothic Neo", sans-serif; }
    ...
    .noti-dropdown { display: none; position: absolute; right: 0; top: 44px; width: 320px; background: #fff; border: 1px solid var(--border); border-radius: 10px; box-shadow: 0 10px 25px rgba(0,0,0,0.15); z-index: 100; overflow: hidden; }
...
<body>
  ...
  <div class="notice-section">
    <div class="notice-header-bar">
      ...
      <button class="btn btn-primary btn-sm" id="btnAdminNotice" style="display:none;" onclick="openModal('adminNoticeModal')">+ 공지사항 작성 (관리자)</button>
    </div>
  ...
  <!-- 관리자 전용 공지 등록 모달 -->
  <div class="modal-overlay" id="adminNoticeModal">
    <div class="modal">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 style="font-size:17px; font-weight:800;">[관리자] 신규 공식 공지 등록</h3>
        <button class="btn btn-outline btn-sm" onclick="closeModal('adminNoticeModal')">닫기</button>
      </div>
      <form onsubmit="handleCreateNotice(event)">
        <div class="form-group">
          <label>공지사항 제목</label>
          <input type="text" id="newNoticeTitle" placeholder="예: [안내] 지자체 거주요건 개정 공지" required>
        </div>
        <div class="form-group">
          <label>공지 상세 내용</label>
          <textarea id="newNoticeContent" style="height: 120px;" placeholder="공지할 내용을 입력하세요." required></textarea>
        </div>
        <button type="submit" class="btn btn-primary" style="width:100%; padding:10px;">실시간 공지 발행</button>
      </form>
    </div>
  </div>
  ...
  <div class="modal-overlay" id="loanModal">
    <div class="modal">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 style="font-size:18px; font-weight:800;">개인택시 양수자금 제휴대출</h3>
        <button class="btn btn-outline btn-sm" onclick="closeModal('loanModal')">닫기</button>
      </div>
      <div class="escrow-panel">
        <div class="calc-row"><span>대출 가능 한도</span><strong>최대 1억 2,000만 원</strong></div>
        <div class="calc-row"><span>최저 적용 금리</span><strong style="color:var(--primary);">연 4.2% ~ (변동)</strong></div>
        <div class="calc-row total"><span>취급 제휴은행</span><span>신한·하나·국민은행 택시금융팀</span></div>
      </div>
      <form onsubmit="alert('대출 상담 신청이 완료되었습니다. 담당자가 연락드립니다.'); closeModal('loanModal'); event.preventDefault();">
        <div class="form-group">
          <label>필요 대출 희망금액 (만 원)</label>
          <input type="number" placeholder="예: 8000" required>
        </div>
        <div class="form-group">
          <label>상담 연락처</label>
          <input type="tel" placeholder="01012345678" required>
        </div>
        <button type="submit" class="btn btn-primary" style="width:100%; padding:10px;">1:1 한도조회 상담신청</button>
      </form>
    </div>
  </div>

  <!-- 에스크로 3단계 안전결제 모달 (양수인 전용) -->
  <div class="modal-overlay" id="escrowModal">
    <div class="modal">
      ...
      <div class="escrow-step-bar">
        <div class="escrow-step-chip active" id="chip1">1단계: 가계약금</div>
  ...
  <div class="modal-overlay" id="authModal">
    <div class="modal">
      ...
      <form id="loginForm" onsubmit="handleLogin(event)">
        <div class="form-group">
          <label>아이디</label>
          <input type="text" id="loginId" placeholder="아이디 입력 (관리자는 admin1234)" required>
        </div>
        <div class="form-group">
          <label>비밀번호</label>
  ...
  <div class="modal-overlay" id="saleModal">
    <div class="modal">
      <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
        <h3 style="font-size:17px; font-weight:800;">면허 판매글 등록</h3>
        <button class="btn btn-outline btn-sm" onclick="closeModal('saleModal')">닫기</button>
      </div>
      <form onsubmit="handleCreateListing(event)">
        <div class="form-group">
          <label>관할 지자체</label>
          <select id="saleRegion"><option>서울 관할</option><option>경기 수원</option><option>경기 성남</option><option>인천 관할</option></select>
        </div>
        <div class="form-group">
        ...
        <button type="submit" class="btn btn-primary" style="width:100%; padding:10px;">실시간 등록</button>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    let currentUser = JSON.parse(localStorage.getItem('taximoa_user_v5') || 'null');
    let currentListings = [];
    let currentView = 'listings'; // 'listings' | 'chat'
    let currentEscrowItem = null;

    // 알림 리스트
    let notifications = JSON.parse(localStorage.getItem('taximoa_notifications') || '[]');
    ...
    function addNotification(text) {
      ...
      localStorage.setItem('taximoa_notifications', JSON.stringify(notifications));
      renderNotifications();
    }
    ...
    function renderNotifications() {
      ...
      notifications.forEach(n => {
        ...
        item.style.fontWeight = n.unread ? '700' : 'normal';
        item.innerHTML = `<div>${n.text}</div><div class="noti-time">${n.time}</div>`;
        container.appendChild(item);
      });
    }

    /* =========================================================
       3. 네비게이션 & 뷰 전환 (매물 vs 대화 목록)
    ========================================================= */
    ...
    function updateLayout() {
      ...
      if (!currentUser) {
        ...
        navTabs.innerHTML = `
          ...
          <button class="tab-btn" onclick="openModal('loanModal')">💰 대출센터</button>
        `;
        ...
        if (currentUser.role === '양도인') {
          navHtml += `<button class="btn btn-primary btn-sm" style="margin-left:auto;" onclick="openModal('saleModal')">+ 판매글 등록</button>`;
        }
    ...
    function renderListings() {
      ...
      currentListings.forEach(item => {
        ...
        // [중요 수정] 버튼 분기: 양도인에게는 안전 에스크로 진행 결제창을 띄우지 않음!
        let actionButtons = '';
        if (!currentUser) {
          ...
          if (currentUser.id === item.seller_id) {
            actionButtons = `<button class="btn btn-outline btn-sm" onclick="openEscrowProcess('${item.id}', true)">내 매물 거래상황 관리</button>`;
          } else {
            actionButtons = `<span style="font-size:12px; color:var(--muted);">타 양도인 매물 (열람 전용)</span>`;
          }
        ...
        card.innerHTML = `
          <div class="listing-top">
            <div>
              <span class="status-badge">${item.step === 0 ? '거래 가능' : item.step + '단계 진행 중'}</span>
          ...
          <div style="margin-top:10px; display:flex; gap:8px;">${actionButtons}</div>
        `;
    ...
    async function loadMyChatRooms() {
      ...
      data.forEach(msg => {
        ...
        if (currentUser.id === sellerId || currentUser.id === buyerId || currentUser.role === '관리자') {
          if (!myChatRooms[msg.room_id]) {
            myChatRooms[msg.room_id] = {
              title: msg.listing_title || '1:1 상담 대화방',
              lastMsg: msg.message,
              time: msg.created_at
            };
          }
      ...
      const roomKeys = Object.keys(myChatRooms);
      ...
      roomKeys.forEach(rId => {
        ...
        item.onclick = () => enterChatRoom(rId, myChatRooms[rId].title);
        item.innerHTML = `
          <div class="title">${myChatRooms[rId].title}</div>
          <div class="last-msg">${myChatRooms[rId].lastMsg}</div>
        `;
        container.appendChild(item);
      });
    }
    ...
    async function loadMessages() {
      ...
      if (!data || data.length === 0) {
        container.innerHTML = `<div style="text-align:center; padding:30px; color:var(--muted); font-size:12px;">안심 대화방이 개설되었습니다. 첫 인사를 남겨보세요!</div>`;
        return;
      }
    ...
    /* =========================================================
       6. 에스크로 프로세스 (양수인 결제 vs 양도인 조회)
    ========================================================= */
    function openEscrowProcess(id, isSellerView) {
      currentEscrowItem = currentListings.find(i => i.id === id);
      if (!currentEscrowItem) return;

      document.getElementById('esTargetTitle').textContent = `[${currentEscrowItem.region}] ${currentEscrowItem.desc_text} (${(currentEscrowItem.price/10000).toLocaleString()}만 원)`;
      const body = document.getElementById('escrowBodyArea');
      const mid = Math.round(currentEscrowItem.price * 0.1);
      const bal = currentEscrowItem.price - 1000000 - mid;

      if (isSellerView) {
        // 양도인용: 현재 예치 상황 및 인가 확인 안내
        body.innerHTML = `
          <div class="escrow-panel">
            <div style="font-weight:700; color:var(--primary); margin-bottom:6px;">● 현재 에스크로 상태: ${currentEscrowItem.step}단계</div>
            <div class="calc-row"><span>총 매매대금</span><strong>${currentEscrowItem.price.toLocaleString()} 원</strong></div>
            <div class="calc-row"><span>1단계 가계약금 예치 상태</span><span>${currentEscrowItem.step >= 1 ? '✔ 예치완료' : '대기중'}</span></div>
            <div class="calc-row"><span>2단계 중도금/서류 상태</span><span>${currentEscrowItem.step >= 2 ? '✔ 승인완료' : '대기중'}</span></div>
            <div class="calc-row"><span>3단계 최종 정산</span><span>${currentEscrowItem.step >= 3 ? '✔ 정산완료' : '대기중'}</span></div>
          </div>
          <p style="font-size:12px; color:var(--muted);">양수인이 대금을 예치하면 구청 인가 절차 완료 후 등록 계좌로 안전하게 정산됩니다.</p>
        `;
      } else {
        // 양수인용: 실제 단계별 예치 버튼
        body.innerHTML = `
          <div class="escrow-panel">
            <div class="calc-row"><span>총 매매대금</span><strong>${currentEscrowItem.price.toLocaleString()} 원</strong></div>
            <div class="calc-row" style="color:var(--primary); font-weight:700;"><span>1단계 가계약금</span><span>1,000,000 원</span></div>
            <div class="calc-row"><span>2단계 중도금 (10%)</span><span>${mid.toLocaleString()} 원</span></div>
            <div class="calc-row"><span>3단계 잔금</span><span>${bal.toLocaleString()} 원</span></div>
          </div>
          <button class="btn btn-primary" style="width:100%; padding:10px;" onclick="advanceStep(1)">가계약금 100만 원 예치 완료</button>
        `;
      }
    ...
    async function advanceStep(nextStep) {
      await supabaseClient.from('listings').update({ step: nextStep }).eq('id', currentEscrowItem.id);
      addNotification(`[에스크로] [${currentEscrowItem.region}] 매물에 ${nextStep}단계 결제가 진행되었습니다.`);
      alert(`에스크로 ${nextStep}단계가 완료되었습니다!`);
    ...
    async function handleCreateListing(e) {
      ...
      const desc_text = document.getElementById('saleDesc').value;
      ...
      e.target.reset();
      addNotification(`[매물 등록] [${region}] ${(price/10000).toLocaleString()}만 원 신규 매물이 등록되었습니다.`);
      alert('매물이 등록되었습니다!');
      fetchListings();
    }

    /* =========================================================
       7. 공지사항 아코디언 로드 & 관리자 등록
    ========================================================= */
    ...
    async function handleCreateNotice(e) {
      ...
      const { error } = await supabaseClient.from('notices').insert([{
        title, content, author: '대표 관리자'
      }]);

      if (error) { alert('공지 등록 오류: ' + error.message); return; }
      closeModal('adminNoticeModal');
      e.target.reset();
      addNotification(`[공지사항] "${title}" 새 공지가 등록되었습니다.`);
      alert('신규 공지가 등록되었습니다.');
      fetchNotices();
    }

    /* =========================================================
       8. [개편] 관리자 ID/PW 고정 로그인 & 일반 회원가입
    ========================================================= */
    ...
    function checkDuplicateId() {
      ...
      if (id.length < 4) {
        ...
        status.textContent = '✕ 4자 이상 입력해 주세요.';
        isIdChecked = false;
        return;
      }
      if (id === 'admin1234') {
        status.style.color = 'red';
        status.textContent = '✕ 예약된 관리자 아이디입니다.';
        isIdChecked = false;
        return;
      }
      const users = JSON.parse(localStorage.getItem('taximoa_all_users_v5') || '{}');
      if (users[id]) {
        status.style.color = 'red';
    ...
    function verifyAuthSms() {
      ...
      if (code && code === mockSmsCode) {
        isSmsPassed = true;
        status.style.color = 'green';
        status.textContent = '✔ 휴대폰 본인인증이 완료되었습니다.';
      } else {
        isSmsPassed = false;
        status.style.color = 'red';
    ...
    function handleRegister(e) {
      ...
      localStorage.setItem('taximoa_all_users_v5', JSON.stringify(users));

      currentUser = users[id];
      localStorage.setItem('taximoa_user_v5', JSON.stringify(currentUser));
    ...
    // [핵심] 로그인: admin1234 / admin9999 관리자 분기
    ...
    function handleLogout() {
      currentUser = null;
      localStorage.removeItem('taximoa_user_v5');
      activeRoomId = null;
      switchView('listings');
    }

    /* =========================================================
       9. Supabase Realtime 감지
    ========================================================= */
    function subscribeRealtime() {
      supabaseClient.channel('realtime_channel_v5')
        ...
        .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'chat_messages' }, payload => {
          ...
          if (currentUser && payload.new.sender_id !== currentUser.id) {
            addNotification(`[채팅] ${payload.new.sender_name}: ${payload.new.message}`);
          }
          if (activeRoomId && payload.new.room_id === activeRoomId) {
            loadMessages();
          }
          if (currentView === 'chat') {
            loadMyChatRooms();
