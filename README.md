...
<body>
    ...
    <div id="viewListings">
      <div class="card">
        <div class="card-title">
          ...
          <span style="font-size: 11px; color: var(--accent);">● 실시간 통신 활성</span>
        </div>
          ...
          <div class="chat-room-view">
            ...
            <div class="chat-messages" id="chatMessageList">
              <div style="text-align: center; padding: 60px 0; color: var(--muted); font-size: 13px;">
                대화방을 선택하거나 매물에서 <strong>[1:1 문의 채팅]</strong>을 눌러주세요.
              </div>
            </div>
  ...
  <div class="modal-overlay" id="saleModal">
    <div class="modal">
      ...
      <form onsubmit="handleSaveListing(event)">
        ...
        <button type="submit" class="btn btn-primary" style="width:100%; padding:10px;" id="btnSaleSubmit">등록 완료</button>
      </form>
    </div>
  </div>
  ...
  <script>
    ...
    let currentUser = JSON.parse(localStorage.getItem('taximoa_user_v7') || 'null');
    let currentListings = [];
    let currentNotices = [];
    ...
    /* 알림 */
    function toggleNotiDropdown() { document.getElementById('notiDropdown').classList.toggle('active'); }
    async function sendNotificationToUser(receiverId, message) {
      if (!receiverId) return;
      await supabaseClient.from('notifications').insert([{
        receiver_id: receiverId, sender_name: currentUser ? currentUser.name : '시스템', message, is_read: false
      }]);
    }
    async function fetchMyNotifications() {
      if (!currentUser) return;
      const { data } = await supabaseClient.from('notifications').select('*').eq('receiver_id', currentUser.id).order('created_at', { ascending: false });
    ...
    function openNoticeModal(mode, noticeId) {
      ...
      if (mode === 'edit') {
        ...
        editId.value = notice.id; formTitle.value = notice.title; formContent.value = notice.content; submitBtn.textContent = '수정 완료';
      } else {
        modalTitle.textContent = '[관리자] 신규 공지 등록';
        editId.value = ''; formTitle.value = ''; formContent.value = ''; submitBtn.textContent = '공지 등록';
      }
    ...
    async function handleSaveNotice(e) {
      ...
      if (editId) {
        await supabaseClient.from('notices').update({ title, content }).eq('id', editId);
        alert('공지사항이 수정되었습니다.');
      } else {
        await supabaseClient.from('notices').insert([{ title, content, author: '대표 관리자' }]);
    ...
    function openSaleModal(mode, listingId) {
      ...
      if (mode === 'edit') {
        ...
        editId.value = item.id; region.value = item.region; price.value = item.price / 10000; desc.value = item.desc_text; submitBtn.textContent = '수정 완료';
      } else {
        modalTitle.textContent = '면허 판매글 등록';
        editId.value = ''; price.value = ''; desc.value = ''; submitBtn.textContent = '실시간 등록';
      }
      openModal('saleModal');
    }
    ...
    async function handleSaveListing(e) {
      ...
      if (editId) {
        await supabaseClient.from('listings').update({ region, price, desc_text }).eq('id', editId);
        alert('매물 정보가 수정되었습니다.');
      } else {
        await supabaseClient.from('listings').insert([{ region, price, desc_text, seller_id: currentUser.id, seller_name: currentUser.name, step: 0 }]);
        alert('새 매물이 등록되었습니다!');
      }
    ...
    async function advanceEscrowStep(nextStep) {
      const { error } = await supabaseClient.from('listings').update({ step: nextStep }).eq('id', currentEscrowItem.id);
      if (error) { alert('진행 오류: ' + error.message); return; }
      const stepNames = {
        1: '1단계 가계약금(100만 원)을 예치했습니다. 거래 락이 체결되었습니다.',
    ...
    async function sendChatMessage() {
      ...
      const { error } = await supabaseClient.from('chat_messages').insert([{
        room_id: activeRoomId, sender_id: currentUser.id, sender_name: currentUser.name, message: text, listing_title: activeListingTitle || '1:1 안심 상담'
      }]);
    ...
    function checkDuplicateId() {
      ...
      if (id.length < 4) {
        status.style.color = 'red'; status.textContent = '✕ 4자 이상 입력해 주세요.'; isIdChecked = false; return;
      }
      if (id === 'admin1234') {
        status.style.color = 'red'; status.textContent = '✕ 예약된 관리자 아이디입니다.'; isIdChecked = false; return;
      }
      const users = JSON.parse(localStorage.getItem('taximoa_all_users_v7') || '{}');
      if (users[id]) {
        status.style.color = 'red'; status.textContent = '✕ 이미 사용 중인 아이디입니다.'; isIdChecked = false;
      } else {
        status.style.color = 'green'; status.textContent = '✔ 사용 가능한 아이디입니다.'; isIdChecked = true;
      }
    }
    ...
    function handleRegister(e) {
      ...
      localStorage.setItem('taximoa_all_users_v7', JSON.stringify(users));
      currentUser = users[id];
      localStorage.setItem('taximoa_user_v7', JSON.stringify(currentUser));
    ...
    function handleLogout() {
      currentUser = null;
      localStorage.removeItem('taximoa_user_v7');
      activeRoomId = null;
      switchView('listings');
      fetchMyNotifications();
    }
    ...
    function subscribeRealtime() {
      supabaseClient.channel('realtime_channel_v7')
